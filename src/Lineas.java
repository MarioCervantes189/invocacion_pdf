...
class Lineas {
    ...
    public void insert(int index,String frase,HashMap<String, List<Integer>> diccionario) {
        List<String> lineaAnterior = new ArrayList<>(lineas);
        String listaPaginas = "";
        for (int i = 0; i < diccionario.get(frase).size(); i++) {
            listaPaginas += diccionario.get(frase).get(i) + ", ";
        }
        try {
